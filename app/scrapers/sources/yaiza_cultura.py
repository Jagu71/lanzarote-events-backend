...
class YaizaCulturaScraper(BaseScraper):
    source_name = "yaiza_cultura"
    source_url = settings.yaiza_cultura_url
    fixture_name = "yaiza_cultura_listing.html"
    ...
    def parse(self, html: str) -> list[RawScrapedEvent]:
        ...
        for article in soup.select("h2.entry-title a[href], h2 a[href]"):
            ...
            container = article.find_parent(["article", "div"]) or article.parent
            ...
            events.append(
                RawScrapedEvent(
                    ...
                    locality="Yaiza",
                    language_origin="es",
                    category_hints=["agenda municipal", "yaiza", "cultura"],
                )
            )
        return events
